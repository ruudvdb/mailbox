/*!
 * @file VL6180X.h
 * @brief Define the infrastructure for the VL6180X class
 * @n This is a VL6180X sensor that supports IIC communication. The IIC address can be changed, default to 0x29. The functions are as follows:
...
#define VL6180X_RESULT_RANGE_RAW                      0x064
...
public:
   ...
   */
  VL6180X(uint8_t iiicAddr = VL6180X_IIC_ADDRESS,TwoWire *pWire=&Wire);
  ...
  /**
   * @brief  Retrieve raw ranging data
   * @return   return ranging data ,uint mm
   */
  uint8_t rangeGetRawMeasurement();
