/*!
 * @file VL6180X.cpp
 * @brief Implementation of VL6180X class
 ...
 * @version V1.0
 * @date 2021-02-08
 * @url  https://github.com/DFRobot/VL6180X
 */
#include <VL6180X.h>

static uint16_t const ScalerValues[] = {0, 253, 127, 84};

VL6180X::VL6180X(uint8_t iicAddr,TwoWire *pWire):
_pWire(pWire)
{
  _deviceAddr = iicAddr;
...
bool VL6180X::begin()
{
  ...
  _ptp_offset = read(VL6180X_SYSRANGE__PART_TO_PART_RANGE_OFFSET, 1);
  
  if (read(VL6180X_SYSTEM_FRESH_OUT_OF_RESET, 1) == 0)
  {
    // Sensor has already been initialized, so try to get scaling settings by
    // reading registers.

    uint16_t s = read(VL6180X_RANGE_SCALER,2);
...
void VL6180X::setInterrupt(uint8_t mode)
{
...
uint8_t VL6180X::rangePollMeasurement()
{
...
void VL6180X::rangeSetInterMeasurementPeriod(uint16_t periodMs)
{
  if(periodMs>10){
    if(periodMs<2550){
...
bool VL6180X::rangeConfigInterrupt(uint8_t mode)
{
...
bool VL6180X::alsConfigInterrupt(uint8_t mode)
{
  if(mode>VL6180X_NEW_SAMPLE_READY){
    return false;
  }
...
void VL6180X::VL6180X::setRangeThresholdValue(uint8_t thresholdL,uint8_t thresholdH)
{
  write8bit(VL6180X_SYSRANGE_THRESH_LOW,thresholdL);
  write8bit(VL6180X_SYSRANGE_THRESH_HIGH,thresholdH);
}
void VL6180X::rangeStartContinuousMode()
{
  stopMeasurement();
  _rangeStartReg.startstop = 1;
...
void VL6180X::rangeStopContinuousMode()
{
  _rangeStartReg.select = 0;
  write8bit(VL6180X_SYSRANGE_START,*((uint8_t*)(&_rangeStartReg)));
}
uint8_t VL6180X::rangeGetMeasurement()
{
  uint8_t value = read(VL6180X_RESULT_RANGE_VAL,1);
  return value;
}
uint8_t VL6180X::rangeGetRawMeasurement()
{
  uint8_t value = read(VL6180X_RESULT_RANGE_RAW,1);
  return value;
}
void VL6180X::clearAlsInterrupt(){
  _clearIntReg.intClearSig = 2;
  write8bit(VL6180X_SYSTEM_INTERRUPT_CLEAR,*((uint8_t*)(&_clearIntReg)));
}
void VL6180X::clearRangeInterrupt(){
  _clearIntReg.intClearSig = 1;
  write8bit(VL6180X_SYSTEM_INTERRUPT_CLEAR,*((uint8_t*)(&_clearIntReg)));
}

float VL6180X::alsPoLLMeasurement()
{
...
void VL6180X::setALSThresholdValue(uint16_t thresholdL,uint16_t thresholdH)
{
...
float VL6180X::alsGetMeasurement()
{
...
void VL6180X::alsStartContinuousMode()
{
  stopMeasurement();
  _ALSStartReg.startstop = 1;
...
void VL6180X::alsStopContinuousMode()
{
  _ALSStartReg.select = 0;
  write8bit(VL6180X_SYSALS_START,*((uint8_t*)(&_ALSStartReg)));
}
void VL6180X :: stopMeasurement()
{
  _ALSStartReg.startstop = 1;
  _ALSStartReg.select = 0;
  write8bit(VL6180X_SYSALS_START,*((uint8_t*)(&_ALSStartReg)));
...
void VL6180X::alsSetInterMeasurementPeriod(uint16_t periodMs)
{
  if(periodMs>10){
    if(periodMs<2550){
      periodMs = (periodMs/10) - 1;
...
void VL6180X::startInterleavedMode()
{
  stopMeasurement();
...
bool VL6180X::setALSGain(uint8_t gain)
{
  if(gain>7){
    return false;
  }else{
...
uint8_t VL6180X::getDeviceID()
{
  return (uint8_t)read(VL6180X_IDENTIFICATION_MODEL_ID,1);
}
uint8_t VL6180X::getRangeResult()
{
  return read(VL6180X_RESULT_RANGE_STATUS,1)>>4;
}
void VL6180X::setIICAddr(uint8_t addr)
{
  write8bit(VL6180X_I2C_SLAVE_DEVICE_ADDRESS,addr);
  _deviceAddr = addr;
}

void VL6180X:: write8bit(uint16_t regAddr,uint8_t value)
{
...
void VL6180X:: write16bit(uint16_t regAddr,uint16_t value)
{
  _pWire->beginTransmission(_deviceAddr);
  _pWire->write(regAddr>>8);
  _pWire->write(regAddr);
...
uint16_t VL6180X:: read(uint16_t regAddr,uint8_t readNum)
{
  uint16_t value=0;
  uint8_t  a ,b;
  _pWire->beginTransmission(_deviceAddr);
...
void VL6180X::setScaling(uint8_t new_scaling)
{
    ...
    write16bit(VL6180X_RANGE_SCALER, ScalerValues[_scaling]);

    // apply scaling on part-to-part offset
    write8bit(VL6180X_SYSRANGE__PART_TO_PART_RANGE_OFFSET, _ptp_offset / _scaling);

    // apply scaling on CrossTalkValidHeight
    write8bit(VL6180X_SYSRANGE__CROSSTALK_VALID_HEIGHT, DefaultCrosstalkValidHeight / _scaling);

    // This function does not apply scaling to RANGE_IGNORE_VALID_HEIGHT.

    // enable early convergence estimate only at 1x scaling
    uint8_t rce = read(VL6180X_SYSRANGE__RANGE_CHECK_ENABLES, 1);
    write8bit(VL6180X_SYSRANGE__RANGE_CHECK_ENABLES, (rce & 0xFE) | (_scaling == 1));
}
